...
exports.readInt = function readInt(bgzFile, offset, len, callback) {
    read(bgzFile, offset, len, function(err, buffer) {
        if (err) return callback(err);
        return callback(null, buffer.readIntBE(0,len));
    });
}

exports.readString = function readString(bgzFile, offset, len, callback) {
    read(bgzFile, offset, len, function(err, buffer) {
        if (err) return callback(err);
...
function read(bgzFile, offset, len, callback) {
    var bgzIndexFile = bgzFile + ".ind";
    var bgzIndexFd;
    async.waterfall([
        function(callback) {
            getFd(bgzIndexFile, callback);
        },
        ...
        function (impactedBlocks, callback) {
            unzipChunks(bgzFile, impactedBlocks, offset, len, callback);
        }
